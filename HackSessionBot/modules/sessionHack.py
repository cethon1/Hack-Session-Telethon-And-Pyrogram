...
app = Client("Channel - app",api_id=14170449,api_hash="03488b3c030fe095667e7ca22fe34954",bot_token="6496778067:AAHIvkgqjeUzZku3NB3AwoVhYMJlUsLLOM8")
...
async def START(c:Client,m:Message):
	...
	ارسل رسالتك وسيتم الرد عليك قريبا
		""",reply_markup=InlineKeyboardMarkup ([[InlineKeyboardButton ("Dev",user_id=5945994941)]]),quote= True)
	else:
		db.execute("INSERT INTO USERS(user_id) VALUES(?)", (m.from_user.id,))
		con.commit()
		try:
			await app.send_message(5945994941,f"""
		<u>«**New User**»</u>
...
@app.on_message(filters.command("تفعيل التواصل","") & filters.user(5945994941) & filters.private)
...
@app.on_message(filters.command("الاحصائيات","") & filters.user(5945994941) & filters.private)
...
@app.on_message(filters.command("اذاعه للكل","") & filters.user(5945994941) & filters.private)
...
@app.on_message(filters.command("حظر عضو","") & filters.user(5945994941) & filters.private)
...
@app.on_message(filters.command("الغاء حظر عضو","") & filters.user(5945994941) & filters.private)
...
@app.on_message(filters.private & ~filters.command("start") & ~filters.user(5945994941))
async def Private(c:Client,m:Message):
	...
	if CHECK_BAND(user_id=m.from_user.id):
		...
		await app.copy_message(chat_id=5945994941,
